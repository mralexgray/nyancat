...
#import "telnet.h" /*!	@discussion telnet.h contains some #defines for the various commands, escape characters, and modes for telnet.
                                    (it surprises some people that telnet is, really, a protocol, and not just raw text transmission)		*/

#import "animation.c" //	The animation frames are stored separately in 	this header so they don't clutter the core source
...
BOOL         telnet = NO,     //	Are we currently in telnet mode?
show_counter = YES,    //	Whether or not to show the counter
clear_screen = YES,    //	Clear the screen between frames (as opposed to reseting the cursor position)
...
void SIGWINCH_handler(int sig) { struct winsize w;
  ...
  ioctl(0, TIOCGWINSZ, &w); terminal_width = w.ws_col; terminal_height = w.ws_row;
...
static void newline(int n) {	for (int i = 0; i < n; ++i) { //	Telnet requires us to send a specific sequence for a line break (\r\000\n), so let's make it happy.
