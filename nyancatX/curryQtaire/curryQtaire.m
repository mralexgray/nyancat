...
void    parseOpts (int argc, char * argv[]); // predeclare
...
BOOL   show_counter = YES,    //	Whether or not to show the counter
         ...
         show_intro = YES;    // right ow, irrelevant, here to calm get_opt
...
BOOL using_automatic_width = 0, //	Flags to keep track of whether width/height were automatically set.
...
int main(int argc, char * argv[]) { @autoreleasepool {
  ...
  if (min_col == max_col) {	using_automatic_width = YES;

    min_col = (FRAME_WIDTH - AtoZ.terminal_width/2) / 2;
    max_col = (FRAME_WIDTH + AtoZ.terminal_width/2) / 2;

  }
  if (min_row == max_row) { using_automatic_height = YES;
  ...
  if (set_title) {                                      	/* Attempt to set terminal title */
    printf("\033kHUGE\033\134");
    printf("\033]1;NVageenya...\007");
    printf("\033]2;Ny  whatever ya...\007");
  }
  ...
  __block char last = 0;      /* Last color index rendered */ //	int y, x; coordinates of what we're drawing */
  while (playing) {           //    IterateGridWithBlock( $RNGTOMAX(min_row, max_row), $RNGTOMAX(min_col, max_col), ^(NSI y, NSI x) {

    reset_cursor();                                     /// STEP 1    Reset cursor
                                                        /// STEP 2    Render the frame */
    for (int y = min_row; y < max_row; y++) {           //  STEP 2.1  iterate rows
      for (int x = min_col; x < max_col; ++x) {         //  STEP 2.2  iterate cols
        char color = y > 23 &&                          //  STEP 2.3  Generate the rainbow tail.
                     y < 43 &&                          //            This is done with a pretty simplistic square wave.
                     x < 0   ? ({
...
void    parseOpts (int  argc, char * argv[]) {
  ...
  while ((c = getopt_long(argc,argv,"eshiItnf:r:R:c:C:W:H:",long_opts, &opt_index)) != -1) {    /* Process arguments */
    c = c ?: !long_opts[opt_index].flag ? long_opts[opt_index].val : c;
    switch (c) {
      case 'e':
        clear_screen = 0;
        break;
