...
char       * output = "  ";        
int          telnet = 0,      /*	Are we currently in telnet mode?		*/
       show_counter = 1,      /*	Whether or not to show the counter		*/
        frame_count = 0,      /*	Number of frames to show before quitting or 0 to repeat forever (default)		*/
       clear_screen = 1,      /*	Clear the screen between frames (as opposed to reseting the cursor position)		*/
          set_title = 1;      /*	Force-set the terminal title.		*/
...
int digits(int val) {	int d = 1, c;	if (val >= 0) for (c = 10; c <= val; c *= 10) d++; else for (c = -10 ; c >= val; c *= 10) d++; return (c < 0) ? ++d : d; }
...
int terminal_width = 80, terminal_height = 24;
/*	Flags to keep track of whether width/height were automatically set.		*/
char using_automatic_width = 0, using_automatic_height = 0;
/*	Print escape sequences to return cursor to visible mode	and exit the application.		*/
void finish() {	printf(clear_screen ? "\033[?25h\033[0m\033[H\033[2J":"\033[0m\n"); exit(0); }
...
void SIGWINCH_handler(int sig) {
	struct winsize w;	ioctl(0, TIOCGWINSZ, &w);	terminal_width = w.ws_col; terminal_height = w.ws_row; 
  
	if (using_automatic_width)  { min_col = (FRAME_WIDTH - terminal_width/2) / 2; max_col = (FRAME_WIDTH + terminal_width/2) / 2; }
	if (using_automatic_height) { min_row = (FRAME_HEIGHT - (terminal_height-1)) / 2;	max_row = (FRAME_HEIGHT + (terminal_height-1)) / 2; }
}

/*	Telnet requires us to send a specific sequence for a line break (\r\000\n), so let's make it happy.		*/
static void newline(int n) {
	
	for (int i = 0; i < n; ++i) {	if (!telnet) { putc('\n', stdout); continue; } /* Send a regular line feed */
...
unsigned char telnet_options[256] = { 0 }, telnet_willack[256] = { 0 };
/*	These are the values we have set or agreed to during our handshake. These are set in send_command(...)		*/
unsigned char telnet_do_set[256]  = { 0 }, telnet_will_set[256]= { 0 };
...
int main(int argc, char * argv[]) { @autoreleasepool {             
  ...
  printf(clear_screen?"\033[H\033[2J\033[?25l": "\033[s"); 		/* Clear the screen */
	...
	if (show_intro) {           /* Display the MOTD */
		...
		}   clear_screen ? printf("\033[H\033[2J\033[?25l") : (void)nil;
	}
