		...
		for ( x=0; x < i->width; ++x ) {
			...
			while ( src <= src_end ) {
				...
				if ( components != 3 )
					...
					v += jp_RED[src[0]] +jp_GREEN[src[1]] + jp_BLUE[src[2]];
...
void         free_image(Image* i) {
	...
	if ( i->pixel ) free(i->pixel);   if ( i->red ) free(i->red);
	if ( i->green ) free(i->green);   if ( i->blue ) free(i->blue);
	if ( i->yadds ) free(i->yadds);   if ( i->lookup_resx ) free(i->lookup_resx);
}
void       malloc_image(Image* i) {
	...
	if ( !(i->pixel && i->yadds && i->lookup_resx) || (usecolors && !(i->red && i->green && i->blue)) )
	{
...
void         decompress(FILE *fp, FILE *fout) {
	...
	int row_stride;	struct jpeg_error_mgr jerr;	struct jpeg_decompress_struct jpg;
	JSAMPARRAY buffer;	Image image;
	...
	if ( jpg.data_precision != 8 ) 
		return (void)fprintf(stderr,	"Image has %d bits color channels, we only support 8-bit.\n",	jpg.data_precision);
		// exit(1);
	...
	if ( clearscr ) { fprintf(fout, "%c[2J", 27);     // ansi code for clear
  ...
  printf("pringing Image %i x %i\n", jpg.output_width,jpg.output_height );
	...
	free_image(&image);
	jpeg_finish_decompress(&jpg);	jpeg_destroy_decompress(&jpg);
}
