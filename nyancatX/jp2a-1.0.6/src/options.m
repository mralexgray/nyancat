...
float jp_RED[256],jp_GREEN[256], jp_BLUE[256], jp_GRAY[256];

const char *fileout = "-"; // stdout

const char* version   = "jp2a 1.0.6";
...
void precalc_rgb(const float red, const float green, const float blue) {
	int n;
	for ( n=0; n<256; ++n ) {
		jp_RED[n]   = ((float) n) * red / 255.0f;
		jp_GREEN[n] = ((float) n) * green / 255.0f;
		jp_BLUE[n]  = ((float) n) * blue / 255.0f;
		jp_GRAY[n]  = ((float) n) / 255.0f;
	}
}

#define PARSE(...)  do { parse_options(__VA_ARGS__); } while (0)

void parse_optionsList(va_list ap) {}


void parse_options(int argc, char** argv) { // (va_list ap){ // int argc, char** argv) {
	...
	if ( (int)((redweight + greenweight + blueweight)*10000000.0f) != 10000000 ) {
		fputs("Weights jp_RED +jp_GREEN + BLUE must equal 1.0\n", stderr);
		exit(1);
	}
