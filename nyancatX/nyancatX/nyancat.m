...
- (void) play {
  ...
  time_t start, __unused current;  time(&start);
  ...
  while (playing) {
    _clear_screen ? printf("\033[H") : printf("\033[u");   //	 Reset cursor 
    for (y = min_row; y < max_row; ++y) {                  //	 Render the frame 
      for (x = min_col; x < max_col; ++x) {                //  if (always_escape) printf("%s", colors[frames[i][y][x]]); /* Text mode (or "Always Send Color Escapes") */  else {
      
          if (frames[i][y][x] != last && self.colors[frames[i][y][x]]) {
            last = frames[i][y][x];                                       //	 Normal Mode, send escape (because the color changed) 
            printf("%s%s", &self.colors[frames[i][y][x]], self.output);
          } else 
          printf("%s", self.output);                               //	 Same color, just send the output characters
//        }
      }
...
/**
    ...
    */
    ...
    if (self.frame_count != 0 && f == self.frame_count)
      [self finish];
    ++i;
